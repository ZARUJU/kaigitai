...
| sources     | 任意 | List[Source]                           | 先頭要素のurl必須（Level1） |  |
...
- MEETINGのみ拡張：`data/meeting/{uuid}/` 配下に複数ファイルで分割する
  - `data/meeting/{uuid}/basic.json`：`id`、`main`、`sub`、`date`、`holding`、`start_time`、`end_time`、`agenda`、`sources`（会議ページまたは開催案内）、`materials` を格納
